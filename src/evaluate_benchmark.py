...
VQAv2 and COCO are the SECONDARY preservation/benchmark check, evaluated with
the GENERIC prompt. Data and predictions are kept in separate directories:
...
Note: VQAv2 has no local scorer — it produces a prediction file for EvalAI
submission. COCO captions are scored locally with score_captions.py.
...
# A) Base mix model (bf16, generic prompt) — both tasks, model loaded once
...
# B) LoRA local adapter (bf16, generic prompt)
python src/evaluate_benchmark.py \
...
# C) QLoRA local adapter (4bit, generic prompt)
python src/evaluate_benchmark.py \
...
def load_model(model_id: str, base_model_id: str, precision: str = "bf16"):
    ...
    # 2. Configure precision: 4-bit (QLoRA) or bf16 (LoRA/base)
    if precision == "4bit":
...
def download_and_extract(url: str, dest_folder: str) -> None:
    ...
    # Skip extraction if a marker shows this zip was already extracted
...
def run_vqa(model, processor, args):
    print("\n" + "=" * 70)
    print(f"🚀 VQA v2 PREDICTION — {args.model_id}")
    print("=" * 70)
    ...
    for idx, item in enumerate(tqdm(questions_list)):
        ...
        prompt = build_vqa_prompt(question_text, args.prompt_style)
...
def run_caps(model, processor, args):
    print("\n" + "=" * 70)
    print("🚀 COCO CAPTIONS VALIDATION PREDICTIONS")
    print("=" * 70)
    print(f"📂 Model     : {args.model_id}")
    ...
    # ── Data setup ────────────────────────────────────────────────────────────
    # Captions are reported on the COCO VALIDATION set (the test server is no
    # longer available). annotations_trainval2014.zip provides captions_val2014.json
    # which contains BOTH the image list (image_id <-> file_name) AND the
    # reference captions used for scoring with pycocoevalcap.
    data_root = os.path.join(args.workdir, "coco_data")
    os.makedirs(data_root, exist_ok=True)

    print("\n🏗️  Setting up COCO Captions (validation) data...")
    ...
    VAL_CAPS_PATH  = os.path.join(data_root, "annotations", "captions_val2014.json")

    # ── Load image list (from the val captions file) ──────────────────────────
    print(f"\n📋 Loading validation image list from {VAL_CAPS_PATH}...")
    with open(VAL_CAPS_PATH) as f:
        val_info = json.load(f)

    test_images = val_info["images"]
    print(f"   ✅ Loaded {len(test_images):,} validation images")
