...
-----
  ...
  * Both models are evaluated at bf16 with a visual-input budget matched to
    PaliGemma's fixed 256 tokens: Qwen2.5-VL is pinned to exactly
    --visual_tokens (256) per image, and SmolVLM2's longest edge is capped at
    --longest_edge (768px, ~324 tokens; its tiling cannot produce exactly 256).
    Raise both to compare at each model's native resolution instead.
  * Predictions are checkpointed every 50 batches via an atomic rename, so an
    interrupted run resumes without a corrupted file.
"""
...
# VQAv2 validation: answers ARE public, so a subset can be scored locally.
# (test2015 answers are not released, so a test-std subset cannot be scored.)
VQAV2_VAL_IMAGES_URL    = "http://images.cocodataset.org/zips/val2014.zip"
VQAV2_VAL_QUESTIONS_URL = "https://s3.amazonaws.com/cvmlp/vqa/mscoco/vqa/v2_Questions_Val_mscoco.zip"
VQAV2_VAL_ANNOT_URL     = "https://s3.amazonaws.com/cvmlp/vqa/mscoco/vqa/v2_Annotations_Val_mscoco.zip"

IMG_TPL = {"test": "COCO_test2015_%012d.jpg", "val": "COCO_val2014_%012d.jpg"}
...
# Visual-input budget, set from the CLI in main() before a runner is built.
args_visual_tokens = [256]   # Qwen2.5-VL: exact visual-token count per image
args_longest_edge  = [768]   # SmolVLM2 : longest image edge in pixels
...
def caption_instruction(style: str) -> str:
    ...
    return "Describe this scene for a blind person."
...
class SmolVLM2Runner:
    ...
    def __init__(self, model_id, dtype=torch.bfloat16):
        ...
        # SmolVLM2 tiles images into 384x384 patches (~81 visual tokens each
        # after pixel shuffle). Capping the longest edge bounds the tile count;
        # 768 gives ~324 tokens, the closest setting at or above PaliGemma's 256.
        self.processor.image_processor.size = {"longest_edge": args_longest_edge[0]}
...
class Qwen25VLRunner:
    ...
    def __init__(self, model_id, dtype=torch.bfloat16):
        ...
        # Visual-token budget matched to PaliGemma's fixed 256 tokens: setting
        # min_pixels == max_pixels pins every image to exactly 256 tokens,
        # making the comparison equal-budget and much cheaper to run.
        tok = args_visual_tokens[0]
        self.processor = AutoProcessor.from_pretrained(
            model_id,
            min_pixels=tok * 28 * 28,
            max_pixels=tok * 28 * 28,
        )
        self.processor.tokenizer.padding_side = "left"
...
def run_vqa(runner, args):
    ...
    for bi, i in enumerate(tqdm(range(0, len(items), args.batch_size))):
        ...
        if bi % 50 == 0:                       # batch counter: never drifts
            tmp = out_file + ".tmp"            # atomic write: no partial files
...
def run_vqav2(runner, args):
    ...
    if args.vqav2_split == "test":
        ...
        download_and_extract(VQAV2_VAL_IMAGES_URL, data_root)
        download_and_extract(VQAV2_VAL_QUESTIONS_URL, data_root)
        download_and_extract(VQAV2_VAL_ANNOT_URL, data_root)
        images_dir = os.path.join(data_root, "val2014")
        q_path = os.path.join(data_root, "v2_OpenEnded_mscoco_val2014_questions.json")

    questions = json.load(open(q_path))["questions"]
    print(f"{len(questions):,} questions in the {args.vqav2_split} split")

    if args.sample_n and args.sample_n < len(questions):
        rng = random.Random(args.sample_seed)
        questions = sorted(rng.sample(questions, args.sample_n),
                           key=lambda q: q["question_id"])
        print(f"sampled {len(questions):,} questions "
              f"(seed {args.sample_seed}; reproducible)")
    if args.limit:
        questions = questions[:args.limit]

    base = "vqav2_%s%s_predictions.json" % (
        args.vqav2_split, ("_sample%d" % args.sample_n) if args.sample_n else "")
    out_file = _pred_path(args.output_dir, base, args.tag)
    results = []
    if os.path.exists(out_file) and not args.preview:
        results = json.load(open(out_file))
    ...
    for bi, i in enumerate(tqdm(range(0, len(questions), args.batch_size))):
        chunk = questions[i:i + args.batch_size]
        imgs, instrs, keep = [], [], []
        for q in chunk:
            fname = IMG_TPL[args.vqav2_split] % q["image_id"]
            try:
    ...
    json.dump(results, open(out_file, "w"), indent=2)
    print(f"saved {len(results):,} -> {out_file}")
    if args.vqav2_split == "test" and not args.sample_n:
        print("Submit to the VQAv2 test-standard server:")
        print("  https://eval.ai/web/challenges/challenge-page/830/overview")
    else:
        print("Score locally against "
              f"{data_root}/v2_mscoco_val2014_annotations.json")
    return out_file
...
def main():
    ...
    ap.add_argument("--vqav2_split", choices=["test", "val"], default="test",
                    help="test = test-standard (server submission, answers not public); "
                         "val = validation (answers public, scorable locally)")
    ap.add_argument("--sample_n", type=int, default=0,
                    help="evaluate a random sample of N questions (0 = all). "
                         "Use with --vqav2_split val so the sample can be scored.")
    ap.add_argument("--sample_seed", type=int, default=20260819,
                    help="seed for --sample_n, so the subset is reproducible")
    ap.add_argument("--visual_tokens", type=int, default=256,
                    help="Qwen2.5-VL: exact visual tokens per image "
                         "(256 matches PaliGemma; higher is slower)")
    ap.add_argument("--longest_edge", type=int, default=768,
                    help="SmolVLM2: longest image edge in pixels "
                         "(768 ~ 324 visual tokens; 1536 is the model default)")
    ...
    args_visual_tokens[0] = args.visual_tokens
    args_longest_edge[0] = args.longest_edge
    print(f"visual budget: Qwen {args.visual_tokens} tokens/image | "
          f"SmolVLM2 longest edge {args.longest_edge}px")
