...
def build_vqa_samples(vqa_ann: list[dict], image_index: dict) -> list[dict]:
...
def main() -> None:
    ...
    parser.add_argument("--workdir",  default="data/vizwiz",      help="Download cache directory")
    ...
    # --- Training set (VQA + captions) --------------------------------------
    print("\nBuilding training set …")
    train_samples  = build_vqa_samples(train_vqa, train_index)
    train_samples += build_caption_samples(
        train_cap, train_index, args.train_captions_per_image
    )
    ...
    val_samples  = build_vqa_samples(val_vqa, val_index)
    val_samples += build_caption_samples(
        val_cap, val_index, args.val_captions_per_image
    )
