...
train_qlora.py
========
QLoRA fine-tuning of PaliGemma-3B
...
def main() -> None:
    ...
    parser.add_argument("--base_output_dir",         default="outputs/qlora")
    ...
    # Per-run output dir + auto-scaled alpha (mirrors train_lora.py)
    ...
    info("Alpha",          f"{lora_alpha}  (rank x 2)")
    ...
    trainer = Trainer(
        ...
        # Early stopping disabled: both runs train the full num_epochs and the
        # best-eval-loss checkpoint is selected via load_best_model_at_end.
